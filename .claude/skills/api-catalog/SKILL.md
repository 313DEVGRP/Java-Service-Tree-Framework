...
description: >-
  ...
  Backend-Core 는 컨트롤러 92개 중 58개가 TreeAbstractController 를 상속해 엔드포인트가 선언 359개 → 실제 1,180개로 늘어나므로
...
PDF 는 `python-markdown → HTML → Edge 헤드리스` 로 낸다 (이 PC 에 pandoc·wkhtmltopdf 없음).

```bash
python $S/render.py "$CATALOG_TASK/artifacts/<name>.md" /tmp/x.html "제목" "부제" "v0.1"
...
| **Global-Config** | 규모가 작고 체인이 깊다 (엔드포인트 33에 체인 노드 195) | Feign 2개 모두 실제 호출됨 |
| **Broker-Hub** | **가장 작다** — 컨트롤러 8개에 엔드포인트 5개뿐 | 컨트롤러 수보다 엔드포인트가 적다. Feign 2개 선언돼 있으나 **호출 노드 0** — 실제 미사용인지 확인이 필요하다 |
