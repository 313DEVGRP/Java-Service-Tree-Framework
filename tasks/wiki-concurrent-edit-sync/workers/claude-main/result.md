# Result — claude-main / wiki 동시 편집 동기화 불일치
...
현재 diff 경로는 이 값을 읽지 않아 증상은 없지만, `EditorController.java:216-222`가 `broadcastFullDocumentState`로 이 null을 계속 내보내고 있다. 서버 권위 상태를 도입하려는 어떤 수정도 여기서 먼저 막힌다.
