...
## [2026-09-21] [arms-aiagent-req-definition]
**교훈**: 엑셀을 산출물로 다루는 작업에서 openpyxl 은 **x14 확장 데이터 유효성(dataValidation)을 저장 시 조용히 제거**한다. 시트 간 참조 드롭다운은 대개 이 확장 형식이라, 값만 바꿔 저장해도 사용자 눈에는 "드롭다운이 사라진" 회귀가 된다. → 엑셀 편집 전 `xl/worksheets/sheet*.xml` 에서 `x14:dataValidation` 유무를 먼저 확인하고, 있으면 동일 참조 범위·문구로 표준 DataValidation 을 재생성한 뒤 저장 후 재열기 검증까지 한 묶음으로 처리할 것. 또한 **양식 파일의 '작성 가이드' 시트와 실제 기존 데이터 관행이 어긋나는 경우가 많다**(여기선 ID 체계 REQ-F-001 vs REQ-AIAGENT-FUNC-01, 상세 블록 4개 vs 7개) — 둘 중 무엇이 정본인지는 추측하지 말고 사용자에게 묻거나, 자율 진행이면 "하위 호환되는 상위 집합"을 고르고 근거를 로그에 남긴다.
**근거**: 저장 후 재열기 검증(C10)에서 DV 3종 보존을 확인. 가이드 4블록 대신 7블록을 택한 근거는 `요구사항_TASK_전환_Format.md` §4 의 기계 변환 전제.
**worker**: orchestrator 단독(worker 호출 없음 — routing.md '복합 작업 우선순위' 2항 적용)

## [2026-09-21] [bash-heredoc-limit]
...
## [2026-09-21] [worker-quota-outage-no-substitution]
**교훈**: 승인된 리뷰 워커가 **쿼터 소진으로 호출 자체가 불가**할 때(codex `You've hit your usage limit`, 회복일이 3주 뒤) 취할 행동은 세 가지뿐이다 — ① 승인 목록 밖 워커로 갈아타기(금지: 승인 게이트 위반) ② 작업 중단 ③ **ERROR 기록 후 리뷰 항목을 "기계 대조 가능분 / 제3자 시각이 필요한 분"으로 갈라, 전자는 Orchestrator 가 직접 수행하고 후자는 미수행으로 남긴 뒤 재개 명령까지 result.md 에 적어두기**. ③ 을 택하되 **"리뷰 통과"로 적지 말 것** — 자체 대조는 같은 추출물을 근거로 삼으므로 독립성이 없다. `result.md` 제목을 '미수행(ERROR)' 로 두고 대체 조치 표와 미대체 항목을 분리해 적으면, 나중 세션이 status 만 보고 검증됐다고 오인하지 않는다.
**근거**: REQ-BACKEND-FUNC-01 — codex-critic 2회(최초+재시도) 동일 실패. brief 점검 5항목 중 4항목은 CSV↔sources 전수/표본 대조로 대체 가능했고, 컬럼 스펙 타당성과 누락 감사 재현은 대체 불가로 남겼다.
**worker**: orchestrator(대체 대조), codex-critic(호출 실패)

## [2026-09-21] [claude-main-write-policy-split]
**교훈**: `claude-main` 은 파일 쓰기 ❌(Orchestrator 경유)인데, "CSV/문서를 만들어라" 류 작업은 자연스럽게 워커에게 쓰기를 시키게 된다. 정책을 지키면서 워커의 값어치를 살리는 분할은 **"기계적 추출·파일 생성 = Orchestrator / 판단이 드는 부분 = 워커"** 다. 이번엔 워커에게 ① 추출기 정규식 사각의 누락 감사 ② 컬럼 스펙 ③ 변환 코드(텍스트) 를 맡기고, 스크립트 실행과 파일 기록은 Orchestrator 가 했다. 워커가 스크래치패드에서 드라이런까지 해 두면 Orchestrator 실행이 1회에 맞는다.
**근거**: REQ-BACKEND-FUNC-01 — 워커 드라이런 예상치(383행/자체 360/상속 23)와 Orchestrator 실행 결과가 정확히 일치.
**worker**: orchestrator, claude-main(backend-expert 서브에이전트)

## [2026-09-21] [downstream-task-audits-upstream]
**교훈**: A 작업의 산출물을 B 작업의 입력으로 쓰면, **B 가 A 의 감사자가 된다**. REQ-MAPPING-FUNC-01(프론트↔백엔드 대조)이 선행 두 카탈로그의 결함 3종을 드러냈다 — 백엔드 추출기가 선행 슬래시 없는 `@Mapping` 4건을 버린 것, 프론트 추출기가 config 형 호출의 HTTP 메서드를 무조건 GET 으로 채운 것, 그리고 40건이 사실은 제3의 서비스(Middle-Proxy)가 처리한다는 것. 세 가지 모두 A 를 단독으로 검증할 때는 "0건 불일치" 로 통과했다(자기 추출물과의 대조라 순환). → **파생 작업의 검증 결과는 선행 작업으로 되먹임할 것**: 새로 드러난 결함을 선행 task 의 Notes·log 에 후속 과제로 남기고, 고칠지는 사용자 판단으로 넘긴다. 그리고 단독 검증에서 "기준선이 내가 만든 추출물" 이면 그 항목은 통과로 적지 말고 순환임을 명시한다.
**근거**: REQ-BACKEND-FUNC-01 의 체크리스트 10번(own↔sources 완전 일치)은 추출물끼리의 대조였고, 소스 재집계로도 같은 가정을 공유해 4건 누락을 못 잡았다. 매핑 작업에서 프론트가 부르는 실제 URL 과 맞춰보고서야 드러났다.
**worker**: orchestrator(자체 대조·근본원인 추적), claude-main(frontend-expert, 대조 스펙·코드)

## [2026-09-21] [quota-outage-three-in-a-row]
**교훈**: 리뷰 워커가 **회복일이 수주 뒤인 쿼터 소진**에 걸리면, 작업마다 2회씩(최초+재시도) 호출해 매번 같은 에러를 받는 것은 낭비다. 다만 호출 자체를 건너뛰면 "시도했다" 는 기록이 없어 나중에 왜 리뷰가 비었는지 추적이 안 된다. → 절충: **작업당 최초 1회는 반드시 실제로 호출해 에러 원문을 log 에 남기고**, 재시도는 회복 예정일이 오늘보다 뒤면 생략해도 된다(생략 사실을 `[DECISION]` 으로 기록). 세 작업 연속으로 같은 에러를 받은 뒤에야 이 규칙을 정한 것이 늦었다.
**근거**: REQ-BACKEND/FRONTEND/MAPPING-FUNC-01 세 작업 모두 codex-critic 2회 호출 → 동일 `usage limit … try again at Oct 13th, 2026`.
**worker**: orchestrator

## [2026-09-22] [task-folder-loss-is-survivable-if-logic-lives-elsewhere]
**교훈**: `tasks/<task>/` 폴더가 통째로 사라지는 일이 실제로 일어났다(원인 미상, 같은 세션에서 xlsx 2개 소실이 선행). `tasks/*` 는 `.gitignore` 대상이라 **git 으로 복구되지 않고**, 휴지통에도 남지 않는다. 이때 복구 가능성을 가르는 것은 **생성 로직이 어디에 있었느냐** 다 — 산출물 생성기가 작업 폴더 안에만 있었다면 전부 날아갔겠지만, 스킬(`.claude/skills/<skill>/scripts/`, git 추적 대상)로 이관해 둔 뒤라 입력 CSV 2본만으로 동일 산출물을 바이트 단위로 재생성할 수 있었다. 잃은 것은 task.md·log.md·brief·result 뿐. → **작업이 끝나면 생성 로직을 작업 폴더에 두지 말고 추적되는 곳으로 옮길 것.** 그리고 폴더 유실을 발견하면 대화 로그에서 문서를 수작업 복원하지 말 것 — 그건 원문 보존이 아니라 재작성이라 '변경 감지 기준선' 으로서의 신뢰도가 없다. 요구사항 원문을 다시 추출해 재구성하는 편이 정직하다.
**근거**: REQ-MAPPING-FUNC-01 — 재진입 시 폴더 전체 소실 확인. `fe_be_gaps.py`·`sheets_xlsx.py`·`tree_actions.py` 가 스킬에 있어 1,014행 CSV·82시트 엑셀을 그대로 복원, AC 8항목 전수 통과.
**worker**: orchestrator

## [2026-09-22] [dont-write-your-interpretation-into-the-requirement]
**교훈**: 요구사항 원문(엑셀 시트)에 **오케스트레이터의 해석을 써 넣지 말 것.** "menu 별로 나타내면 되며" 를 시트 분할로 해석한 뒤, 그 해석을 시트의 `[입력 / 출력]` 블록에 "메뉴별 시트로 나눈 엑셀 1본" 으로 적어 넣었다. 그러면 ① 사용자가 지시하지 않은 것이 요구사항 정본이 되고 ② `requirement.md` 기준선도 그 문장으로 갱신돼 ③ 나중에 "왜 이렇게 했냐" 를 추적하면 "요구사항에 그렇게 적혀 있다" 는 순환 근거만 남는다. → 시트 원문에는 **사용자 문장과 사용자가 확인한 보완만** 넣고, 해석·판단은 `task.md`(Goal·AC)와 `log.md` `[DECISION]` 에만 남긴다. 요구사항 보완을 요청받았을 때도 '내가 정한 산출 형식' 은 사용자에게 확인하거나 task 쪽에 둔다.
**근거**: REQ-MAPPING-FUNC-01 — 사용자가 "왜 자꾸 시트별로 만드느냐, 요청문에 그렇게 되어 있으면 수정해달라" 고 지적. 확인해보니 원문은 `- 출력 : CSV` 한 줄이었고 나머지는 내가 추가한 문장이었다.
**worker**: orchestrator
